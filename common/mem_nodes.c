...
enum block_status { FREE = 0, USED = 1 };
...
#define MAX_BLOCKS                                                             \
	((BASE_PAGE_SIZE - sizeof(struct block_region)) /                      \
	 sizeof(struct block_wrapper))

#define block_region(b)                                                        \
	((struct block_region *)((size_t)(b) & ~(BASE_PAGE_SIZE - 1)))

#define block_container(b) container_of(b, struct block_wrapper, n)
...
{
	struct block_region *r =
		(struct block_region *)alloc_page(BASE_PAGE, 0);
